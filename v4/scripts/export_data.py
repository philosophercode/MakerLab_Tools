...
def get_tables(config):
    """Get table IDs from config/env, with Cornell MakerLab defaults."""
    def t(key, default):
        return config.get(key) or os.environ.get(key) or default

    return {
        "tools": t("AIRTABLE_TABLE_TOOLS", "tblXHIT0mN2nOzdhd"),
        "categories": t("AIRTABLE_TABLE_CATEGORIES", "tblNpVHquh7H0S8Bc"),
        "locations": t("AIRTABLE_TABLE_LOCATIONS", "tblbwtZhuvtuBKlPO"),
        "units": t("AIRTABLE_TABLE_UNITS", "tblDtKMcCxTyQbXwi"),
        "maintenance_logs": t("AIRTABLE_TABLE_MAINTENANCE_LOGS", "tbl22sgbMLCFbvynl"),
        "flags": t("AIRTABLE_TABLE_FLAGS", "tblAK068QYB0vLopa"),
    }
