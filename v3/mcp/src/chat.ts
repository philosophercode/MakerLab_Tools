...
function buildGeneralSystemPrompt(tools: ResolvedTool[]): string {
...
function buildToolSystemPrompt(
  tool: ResolvedTool,
  docs: Array<{ label: string; url: string; text: string }>
): string {
...
const CHAT_TOOLS: Anthropic.Tool[] = [
  {
    ...
    description:
      "Fetch detailed information and documentation for a specific tool. Use when a student asks detailed questions about a tool.",
    input_schema: {
  ...
  },
  {
    ...
    description:
      "Fetch details for a specific unit (individual machine instance), including status, condition, and maintenance history.",
    input_schema: {
      type: "object" as const,
      properties: {
        unit_label: {
  ...
  },
  {
    name: "report_issue",
    description:
      "Report an equipment issue or maintenance request.",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description:
      "Generate a concept image of a student's project. Write a detailed visual prompt describing the finished object.",
    input_schema: {
      type: "object" as const,
      properties: {
        prompt: {
          type: "string",
          description:
            "Detailed scene description: finished object, materials, colors, textures, setting, lighting, camera angle.",
        },
      },
      required: ["prompt"],
    },
  },
  {
    ...
    description:
      "Generate a visual step-by-step infographic for a making/building process.",
    input_schema: {
      type: "object" as const,
      properties: {
        title: {
  ...
  switch (name) {
    case "get_tool_details": {
      ...
      const match =
        ...
        resolvedTools.find((t) =>
          t.name.toLowerCase().includes(toolName.toLowerCase())
        );

      if (!match) return JSON.stringify({ found: false, message: `No tool found matching "${toolName}".` });
    ...
    case "get_unit_details": {
      const unitLabel = input.unit_label as string;
      const unit = await getUnit(unitLabel);
      if (!unit) return JSON.stringify({ found: false, message: `No unit found matching "${unitLabel}".` });
      return JSON.stringify({ found: true, ...unit });
    }

    case "report_issue": {
      const { title, description, unit_label, priority, reported_by } =
        input as {
          title: string;
          description: string;
          unit_label?: string;
    ...
    case "generate_infographic": {
      const { title, steps } = input as {
        title: string;
        steps: Array<{ number: number; label: string; detail: string }>;
      };
      try {
...
export async function chat(params: {
  ...
  let resolvedTools: ResolvedTool[] = [];

  if (params.tool_id) {
    // Tool-specific chat
    const tool = await getTool(params.tool_id);
    if (!tool) throw new Error(`Tool not found: ${params.tool_id}`);
    ...
    ).filter(Boolean) as Array<{ label: string; url: string; text: string }>;
  ...
  const collectedImages: ChatResult["images"] = [];
  ...
  const tools = params.tool_id
    ? CHAT_TOOLS.filter(
        (t) => t.name !== "get_tool_details" && t.name !== "visualize_project" && t.name !== "generate_infographic"
      )
    : CHAT_TOOLS;
