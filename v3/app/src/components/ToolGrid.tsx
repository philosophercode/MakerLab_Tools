      ...
      {tools.map((tool, i) => {
        ...
        return (
          ...
          >
            ...
            {/* Name — always visible */}
            <span className="min-w-0 flex-1 text-sm font-medium leading-tight truncate group-hover:text-cornell-red transition-colors">
              {tool.name}
            </span>
            ...
            <div className="flex flex-shrink-0 gap-1 sm:w-24 sm:justify-end">
              {hasSafety ? (
                <>
                  ...
                  {needsTraining && (
                    <span className="rounded-full bg-cornell-red/10 px-1.5 py-0.5 text-[10px] text-cornell-red font-medium">
                      Train
                    </span>
                  )}
                  {needsAuth && (
