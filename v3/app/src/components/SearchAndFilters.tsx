      ...
      >
        {categories.map((cat) => {
          ...
          return (
            ...
            >
              <span className={active ? "text-cornell-red" : "text-muted"}>
                {CATEGORY_ICONS[cat] ?? (
...
function AccordionSection({
  ...
  return (
    <div className="border-b border-card-border last:border-b-0">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between py-3 text-sm font-medium text-foreground hover:text-cornell-red transition-colors"
        aria-expanded={open}
      >
        <span className="flex items-center gap-2">
          {title}
          {count !== undefined && count > 0 && (
            <span className="rounded-full bg-cornell-red/10 px-1.5 py-0.5 text-[10px] font-semibold text-cornell-red">
              {count}
            </span>
          )}
        </span>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-card-border">
          ...
          {totalFilterCount > 0 && (
            <button
              ...
              className="text-xs text-cornell-red hover:text-cornell-dark transition-colors"
            >
